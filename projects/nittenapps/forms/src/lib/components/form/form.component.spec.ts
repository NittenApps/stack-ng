...
const STACK_FORM_SELECTOR = 'nas-form';
const STACK_FIELDS_SELECTOR = 'nas-form > nas-field * > nas-field';
...
export const renderComponent = (inputs: IStackFormInputs, config: any = {}) => {
  inputs = {
    form: new FormGroup({}),
    model: {},
    options: {},
...
describe('StackForm Component', () => {
  describe('fields input', () => {
    it('should render fields', () => {
      ...
      expect(queryAll(STACK_FIELDS_SELECTOR)).toHaveLength(2);
    });

    it('should detect fields changes', () => {
      const { queryAll, setInputs, detectChanges } = renderComponent({ fields: null });

      expect(queryAll(STACK_FIELDS_SELECTOR)).toHaveLength(0);

      setInputs({ fields: [{ key: 'foo' }] });
      detectChanges();

      expect(queryAll(STACK_FIELDS_SELECTOR)).toHaveLength(1);
    });
  ...
  describe('model input', () => {
    ...
    it('should take account of using the emitted modelChange value as model input', () => {
      ...
      expect(form?.get('test')?.value).toEqual('1');
        ...
        fields: [
          {
            ...
            hooks: {
              ...
              // Previous value: 'display: '. Current value: 'display: none'.
  ...
  describe('inline type', () => {
    it('should render inline type', () => {
      const { query } = renderComponent(
        ...
        {
          template: `
            <nas-form [fields]="fields">
              <ng-template nasFormTemplate="inline-type" let-field>
    ...
    it('should override existing type', () => {
      const { query } = renderComponent(
        ...
        },
        {
          template: `
            <nas-form [fields]="fields">
              <ng-template nasFormTemplate="input" let-field>
    ...
    it('should override default nas-form-group', () => {
      const { query, queryAll } = renderComponent(
        {
          fields: [
            {
        ...
        },
        {
          template: `
            <nas-form [fields]="fields">
              <ng-template nasFormTemplate let-f>
                <div class="inline-group">
                  <nas-field *ngFor="let field of f.fieldGroup" [field]="field"></nas-field>
                </div>
              </ng-template>
            </nas-form>
          `,
      ...
      expect(query('nas-form-group')).toBeNull();
    });
  });
  ...
  it('should check expression on submit', () => {
    ...
    query('form').triggerEventHandler('submit', null);
    detectChanges();
  ...
  it('should keep disabled state of field group consistent after detect changes', () => {
    const { fields, setInputs, detectChanges } = renderComponent({
      fields: [
        {
          key: 'f1',
          fieldGroup: [
    ...
    expect(field.fieldGroup?.[1].props?.disabled).toBeFalse();
  ...
  it('should detect changes when building a child field', () => {
    ...
    // Previous value for 'disabled': 'false'. Current value: 'true'
