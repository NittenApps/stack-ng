...
import { FieldArrayType, StackFieldConfig, StackFieldProps } from '@nittenapps/forms';
import { createComponent, StackInputModule, ɵCustomEvent } from '@nittenapps/forms/testing';
import { JSONSchema7 } from 'json-schema';
import { StackFormsJsonSchema } from './stack-forms-json-schema.service';

const renderComponent = ({ schema, model }: { schema: JSONSchema7; model?: any }) => {
  const field = new StackFormsJsonSchema().toFieldConfig(schema);

  const options = createComponent<{ field: StackFieldConfig }>({
    template: `
      <form [formGroup]="form">
        <nas-form
          [model]="model"
    ...
    config: {
      types: [
        { name: 'object', extends: 'nas-form-group' },
        { name: 'array', component: ArrayTypeComponent },
        { name: 'multischema', extends: 'nas-form-group' },
...
describe('Service: StackFormsJsonSchema', () => {
  let stackFormsJsonSchema: StackFormsJsonSchema;
  const emmptyFieldProps: StackFieldProps = {
    label: undefined,
    readonly: undefined,
  ...
  beforeEach(() => {
    stackFormsJsonSchema = new StackFormsJsonSchema();
  });
  ...
  describe('keyword support', () => {
    ...
    describe('type keywords', () => {
      it('should guess type as object when properties is set', () => {
        ...
        const config = stackFormsJsonSchema.toFieldConfig(schema);
    ...
    describe('number validation keywords', () => {
      it('should support minimum, maximum and multipleOf', () => {
        ...
        const { props } = stackFormsJsonSchema.toFieldConfig(numSchema);
      ...
      it('should support exclusiveMinimum', () => {
        ...
        const config = stackFormsJsonSchema.toFieldConfig(numSchema);
      ...
      it('should be a number value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'number' });
      ...
      it('should be an integer value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'integer' });
    ...
    describe('null type', () => {
      it('should be a null value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'null' });
    ...
    describe('string validation keywords', () => {
      it('should support pattern', () => {
        ...
        const { props } = stackFormsJsonSchema.toFieldConfig(schema);
      ...
      it('should set nullable string type to `null` if empty', () => {
        ...
        const { parsers } = stackFormsJsonSchema.toFieldConfig(schema);
        const [nullIfEmpty] = parsers!;
      ...
      it('should be a string value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'string' });
      ...
      it('should be a string or null value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: ['string', 'null'] });
    ...
    describe('array validation keywords', () => {
      it('should be an array value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'array' });
      ...
      it('supports array items keyword as object', () => {
        ...
        const config = stackFormsJsonSchema.toFieldConfig(schema);
        const baseConfig: StackFieldConfig = {
          type: 'array',
      ...
      it('supports array items as array of schemas', () => {
        ...
        const childConfig: StackFieldConfig = {
        ...
        const childConfig2: StackFieldConfig = {
          props: { ...emmptyFieldProps, required: true, removable: false },
          type: 'number',
          defaultValue: undefined,
          parsers: [expect.any(Function)],
      ...
      it('supports array additionalitems when array items are defined as an array of schemas', () => {
        ...
        const childConfig3: StackFieldConfig = {
      ...
      it('minItems: should not set default value if array is optional', () => {
        ...
        const config = stackFormsJsonSchema.toFieldConfig(numSchema).fieldGroup?.[0];
      ...
      it('should support uniqueItems', () => {
        ...
        const uniqueItemsValidator = (model: any) =>
          config.validators?.['uniqueItems'](new FormControl(model), { model });
    ...
    describe('object validation keywords', () => {
      it('should be an object value', () => {
        const f = stackFormsJsonSchema.toFieldConfig({ type: 'object' });
      ...
      describe('required keyword', () => {
        ...
        it('required with oneOf/anyOf', () => {
          ...
          expect(cityField?.valid).toBeFalse();
        });
      ...
      describe('dependencies', () => {
        ...
        describe('with schema dependencies', () => {
          it('should display extended schema', () => {
            ...
            const config = stackFormsJsonSchema.toFieldConfig(schema).fieldGroup;
          ...
          it('should display extended schema with oneOf', () => {
            ...
            const [, opt1Field, opt2Field] = stackFormsJsonSchema.toFieldConfig(schema).fieldGroup!;
    ...
    describe('any instance type validation keywords', () => {
      ...
      describe('should support enum type', () => {
        it('should support enum as strig array values', () => {
          ...
          const config = stackFormsJsonSchema.toFieldConfig(schemaStringEnum);

          expect(config.type).toBe('enum');
          expect(config.props?.options).toEqual(enumOptions(schemaStringEnum.enum));

          const config2 = stackFormsJsonSchema.toFieldConfig(schemaNumberEnum);
          ...
          const config3 = stackFormsJsonSchema.toFieldConfig(schemaIntegerEnum);
        ...
        describe('enum as oneOf/anyOf structure', () => {
          it('should support enum as oneOf/const structure', () => {
            ...
            const { type, props } = stackFormsJsonSchema.toFieldConfig(schema);
        ...
        describe('enum as uniqueItems & array structure', () => {
          it('with simple enum item schema definition', () => {
            ...
            const { type, validators, props } = stackFormsJsonSchema.toFieldConfig(numSchema);
          ...
          it('with nested item schema definition', () => {
            ...
            const { type, props } = stackFormsJsonSchema.toFieldConfig(numSchema);
      ...
      it('should support const as hidden', () => {
        ...
        const { type, defaultValue, validators } = stackFormsJsonSchema.toFieldConfig(schema);
      ...
      it('should support const as type', () => {
        ...
        const { type, defaultValue, validators } = stackFormsJsonSchema.toFieldConfig(schema);
        const constValidator = validators?.['const'];
    ...
    describe('Schema Re-Use With "definitions"', () => {
      ...
      it('should resolve a recusive schema definition', () => {
        ...
        const config = stackFormsJsonSchema.toFieldConfig(schema).fieldGroup?.[0];
        const expectedConfig: StackFieldConfig = {
          type: 'array',
          defaultValue: [],
          props: { ...emmptyFieldProps },
          fieldArray: expect.any(Function),
    ...
    describe('Schema allOf support', () => {
      it('should merge allOf array into single object', () => {
        ...
        const expected = field.fieldGroup?.[0].fieldGroup?.map(({ key, type, props }) => ({
          key,
          type,
      ...
      it('should merge required fields', () => {
        ...
        const { fieldGroup } = stackFormsJsonSchema.toFieldConfig(schema);
      ...
      it('should merge allOf with base schema', () => {
        ...
        const expected = field.fieldGroup?.map(({ key, props }) => ({ key, required: props?.required }));
    ...
    describe('Multi-Schema (oneOf, anyOf) support', () => {
      ...
      describe('oneOf', () => {
        ...
        it('should render multischema type when oneOf is present', () => {
          const { fieldGroup } = stackFormsJsonSchema.toFieldConfig(schema);
          const [{ type: oneOfType }] = fieldGroup!;
        ...
        it('should render the valid oneOf field on first render', () => {
          ...
          const [, { fieldGroup }] = field.fieldGroup![0].fieldGroup!;
          const [fooField, barField] = fieldGroup!;
        ...
        it('should render the valid oneOf field when properties have the same name', () => {
          ...
          const [, { fieldGroup }] = field.fieldGroup![0].fieldGroup!;
          const [foo1Field, foo2Field] = fieldGroup!;
        ...
        it('should render the selected oneOf field', () => {
          ...
          const [enumField, { fieldGroup }] = field.fieldGroup![0].fieldGroup!;
          const [fooField, barField] = fieldGroup!;
        ...
        it('should support oneOf within array', () => {
          ...
          const [, { fieldGroup }] = field.fieldGroup![0].fieldGroup![0].fieldGroup!;
          const [foo1Field, foo2Field] = fieldGroup!;
        ...
        it('should support oneOf within array (validate the second item)', () => {
          ...
          const [, { fieldGroup }] = field.fieldGroup![1].fieldGroup![0].fieldGroup!;
      ...
      describe('anyOf', () => {
        ...
        it('should render multischema type when anyOf is present', () => {
          const { fieldGroup } = stackFormsJsonSchema.toFieldConfig(schema);
          const [{ type: anyOfType }] = fieldGroup!;
        ...
        it('should support anyOf using mixed type', () => {
          ...
          const [, { fieldGroup }] = field.fieldGroup![0].fieldGroup![0].fieldGroup!;
          const foo1Field = fieldGroup![0];
          const foo2Field = fieldGroup![1];
    ...
    describe('schema annotations', () => {
      it('should support schema annotations', () => {
        ...
        const { defaultValue, props } = stackFormsJsonSchema.toFieldConfig(schema);
  ...
  describe('widget stackFormsConfig options merging', () => {
    it('should merge a stackFormsConfig object specified in the widget property into the stack forms config', () => {
    ...
    it('should be possible to set the key via stackFormsConfig', () => {
      const schema: JSONSchema7 = JSON.parse(`{
      ...
      const fields = stackFormsJsonSchema.toFieldConfig(schema);
  ...
  describe('StackFormsJsonSchemaOptions map function', () => {
    ...
    it('should not crash when props are not defined', () => {
      ...
      const field = stackFormsJsonSchema.toFieldConfig(schema, {
        map: (field: StackFieldConfig) => {
  ...
  describe('StackFormsJsonSchemaOptions map data', () => {
    ...
    it('should set non required string to undefined when is empty', () => {
      ...
      expect(parser('', field)).toBeUndefined();
...
function getFieldArrayChild(config: StackFieldConfig) {
...
function validateType({ validators }: StackFieldConfig, value: any) {
