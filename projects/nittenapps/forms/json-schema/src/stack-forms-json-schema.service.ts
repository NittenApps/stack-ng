...
interface IOptions extends StackFormsJsonSchemaOptions {
  schema: StackFormsJSONSchema7;
  resetOnHide?: boolean;
...
export interface StackFormsJsonSchemaOptions {
  /**
   * allows to intercept the mapping, taking the already mapped
   * stack forms field and the original JSONSchema source from which it
   * was mapped.
   */
  map?: ((mappedField: StackFieldConfig, mapSource: JSONSchema7) => StackFieldConfig) | null;
}

interface StackFormsJSONSchema7 extends JSONSchema7 {
  widget?: { stackFormsConfig: StackFieldConfig };
}
...
function totalMatchedFields(field: StackFieldConfig): number {
  if (!field.fieldGroup) {
...
export class StackFormsJsonSchema {
  toFieldConfig(schema: JSONSchema7, options?: StackFormsJsonSchemaOptions): StackFieldConfig {
    schema = clone(schema);
    return this._toFieldConfig(schema, { schema, ...(options || {}) });
  }

  private _toFieldConfig(schema: StackFormsJSONSchema7, { key, isOptional, ...options }: IOptions): StackFieldConfig {
    schema = this.resolveSchema(schema, options);
    const types = this.guessSchemaType(schema)!;

    let field: StackFieldConfig & { shareFormControl?: boolean } = {
      type: types[0],
    ...
    switch (field.type) {
      ...
      case 'integer': {
        field.parsers = [
          (v: string | number | null | undefined, f: StackFieldConfig) => {
            v = toNumber(v!);
        ...
        ] as ((value: any, f?: StackFieldConfig) => any)[];
      ...
      case 'array': {
        ...
        if (!this.isEnum(schema)) {
          field.fieldArray = (root: StackFieldConfig) => {
            const length = root.fieldGroup?.length || 0;
    ...
    // map in possible stackFormsConfig options from the widget property
    if (schema.widget?.stackFormsConfig) {
      field = this.mergeFields(field, schema.widget.stackFormsConfig);
    }
  ...
  private addValidator(field: StackFieldConfig, name: string, validator: StackFieldConfig['validators']): void {
    field.validators = field.validators || {};
    field.validators[name] = validator;
  }

  private guessSchemaType(schema: JSONSchema7): JSONSchema7TypeName | JSONSchema7TypeName[] | undefined {
  ...
  private isFieldValid(
    ...
    if (!field) {
      field = schema._field = root.options?.build?.({
        form: Array.isArray(model) ? new FormArray<any>([]) : new FormGroup<any>({}),
        fieldGroup: [
  ...
  private mergeFields(f1: any, f2: any): any {
    for (let prop in f2) {
  ...
  private resolveAllOf({ allOf, ...baseSchema }: StackFormsJSONSchema7, options: IOptions): StackFormsJSONSchema7 {
    if (!allOf?.length) {
      throw Error(`allOf array can not be empty ${allOf}.`);
    }

    return (allOf as StackFormsJSONSchema7[]).reduce((base: StackFormsJSONSchema7, schema: StackFormsJSONSchema7) => {
      schema = this.resolveSchema(schema, options);
  ...
  private resolveDefinition(schema: StackFormsJSONSchema7, options: IOptions): StackFormsJSONSchema7 {
  ...
  private resolveMultiSchema(mode: 'oneOf' | 'anyOf', schemas: JSONSchema7[], options: IOptions): StackFieldConfig {
    return {
      type: 'multischema',
      fieldGroup: [
        {
